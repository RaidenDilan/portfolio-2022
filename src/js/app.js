...
BreakPoints = {
  ...
  currentMajorBreakPoint: function() {
    return 580 >= window.innerWidth ? BreakPoints.MajorBreakPoints.A : 768 >= window.innerWidth ? BreakPoints.MajorBreakPoints.B : 1280 >= window.innerWidth ? BreakPoints.MajorBreakPoints.C : BreakPoints.MajorBreakPoints.D;
  }
};
...
UserAgent = {
  ...
  init: () => {
    ...
    UserAgent.iOS || (
      ...
      UserAgent.android || (
        UserAgent.edge = 0 < a.indexOf('Edge/'),
        UserAgent.edge || (
          UserAgent.internetExplorer = 0 < a.indexOf('Trident/') || 0 < a.indexOf('MSIE ')
        )
      )
    );
  }
...
AboutRAFs = {
  ...
  init: () => {
    ...
    // console.log('Site.scrolling.vars.timer', Site.scrolling.vars.timer);
    ...
    if (Site.scrolling !== null) {
      ...
      if (Math.abs((Site.scrolling.vars.current - Site.deltaScroll) / 200 + 1) < 2.2) {
        // console.log('Site.theRafAbout =-=-=-=-=', Site.theRafAbout);
        Site.intensity = Math.abs((Site.scrolling.vars.current - Site.deltaScroll) / 200 + 1);
      }
      // else if (Math.abs((Site.scrolling.vars.current - Site.deltaScroll)) === 0) {
      //   console.log('2');
    ...
    if (!UserAgent.iOS) {
      TweenMax.to('.scaleA', 1.4, {
        ...
        onComplete: function event() {
          ...
          // console.log('onComplete :: cancelAnimationFrame(Site.theRafAbout) --->');
        }
      });
    ...
    // if (Site.scrolling.vars.current) {
      // cancelAnimationFrame(Site.theRafAbout);
      // console.log('cancelAnimationFrame =-=-=-==--=-');
    // }
    // else Site.theRafAbout = requestAnimationFrame(Site.aboutRafs);
  ...
  showHideArrow: () => {
    if (!Menu.button.classList.contains('opened')) {
      if (!UserAgent.iOS) {
        ...
        if (Site.body.classList.contains('single')) {
          if (window.pageYOffset >= 10) Menu.showArrow();
          else if (window.pageYOffset <= 10) Menu.hideArrow();
          // else Menu.hideArrow();

          if (window.innerHeight + Math.round(window.pageYOffset) >= (document.body.offsetHeight - 34)) Theme.showLightButtonStyle();
          else Theme.showDarkButtonStyle();
...
Drag = {
  ...
  move: (event) => {
    ...
    if ((Site.finishPosition - Site.startPosition < -Drag.threshold) && Site.blockedAction === false) Site.nextSlide();
    else if ((Site.finishPosition - Site.startPosition > Drag.threshold) && Site.blockedAction === false) Site.prevSlide();
...
Site.init = function init() {
  ...
  if (!UserAgent.iOS) window.addEventListener('mousemove', Throttle.actThenThrottleEvents(Site.handlerMouseMove, 500));
  ...
  window.addEventListener('touchmove', Throttle.actThenThrottleEvents(Menu.showHideArrow, 500)); // shows/hides menu arrow when scrolling on mobile devices
  window.addEventListener('wheel', Throttle.actThenThrottleEvents(Menu.showHideArrow, 500));
  window.addEventListener('mousewheel', Throttle.actThenThrottleEvents(Menu.showHideArrow, 500));
  window.addEventListener('DOMMouseScroll', Throttle.actThenThrottleEvents(Menu.showHideArrow, 500));
...
Site.changeProject = function changeProject(event) {
  if (event.target.classList.contains('change_project')) Site.changePagination(event.target);
    ...
    if (Menu.button.classList.contains('opened')) Menu.open();
    else Menu.close();
  }
};
...
Site.resize = function resize() {
  if (!UserAgent.iOS && Site.scrolling !== null) {
    ...
    // console.log('Change SideNav Position');
...
Site.scrollEvent = function scrollEvent(event) {
  ...
  if (Site.lethargy.check(event) !== false && Site.blockedAction === false && !Menu.button.classList.contains('opened') && Site.body.classList.contains('home')) {
